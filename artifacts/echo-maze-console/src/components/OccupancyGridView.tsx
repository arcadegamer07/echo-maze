...
export function OccupancyGridView({ values, mode = 'occupancy', columns = 16, rows = 10, pose, ghostValues, compact = false }: { values: number[]; mode?: CellMapMode; columns?: number; rows?: number; pose?: Pose; ghostValues?: number[]; compact?: boolean }) {
  ...
  const occupancyFill = (value: number) => {
    if (mode === 'diff') return value > .65 ? 'var(--accent)' : value > .28 ? 'var(--amber)' : 'var(--muted-2)';
    return value > .62 ? 'var(--map-ink)' : value > .42 ? 'var(--muted)' : 'var(--surface-3)';
  };
  ...
  return (
    <div className={`grid-viz-shell ${compact ? 'compact' : ''}`}>
      <svg className={`occupancy-grid ${mode === 'diff' ? 'diff-grid' : ''}`} viewBox={`0 0 ${width} ${height}`} role="grid" aria-label={`${mode} occupancy grid`}>
        <defs>
          <pattern id={`minor-${patternId}`} width={cellWidth} height={cellHeight} patternUnits="userSpaceOnUse"><path d={`M ${cellWidth} 0 L 0 0 0 ${cellHeight}`} fill="none" stroke="var(--map-grid)" strokeOpacity=".35" strokeWidth="1" /></pattern>
          <pattern id={`major-${patternId}`} width={cellWidth * 4} height={cellHeight * 4} patternUnits="userSpaceOnUse"><rect width={cellWidth * 4} height={cellHeight * 4} fill="none" stroke="var(--map-grid)" strokeOpacity=".65" strokeWidth="1" /></pattern>
        </defs>
        ...
        {ghostValues?.map((value, index) => value > .56 ? <rect key={`ghost-${index}`} className="ghost-cell" x={(index % columns) * cellWidth + 3} y={Math.floor(index / columns) * cellHeight + 3} width={cellWidth - 6} height={cellHeight - 6} fill="none" stroke="var(--muted)" strokeDasharray="4 3" opacity={Math.min(.55, value * .52)} /> : null)}
