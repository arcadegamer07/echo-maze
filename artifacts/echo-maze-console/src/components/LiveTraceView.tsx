import { Activity, Crosshair, Gauge, MapPin, Radio, Ruler } from 'lucide-react';
      ...
      <PanelBody className="live-trace-body">
        ...
        <div className="live-trace-layout">
          ...
          <div className="live-trace-data">
            ...
            <div className="trace-metric"><span>Current movement</span><strong>{motion.toUpperCase()}</strong><small>{latest ? `${latest.leftSpeed.toFixed(0)} / ${latest.rightSpeed.toFixed(0)} PWM` : 'No command yet'}</small></div>
            ...
            <div className="trace-note"><Gauge size={14} /><span>Dots are real range returns. The line is command-based dead reckoning; without wheel encoders or gyro it is diagnostic, not ground truth.</span></div>
          </div>
        </div>
