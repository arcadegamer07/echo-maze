...
function MapGrid({
  ...
  const focusedX = focused === null ? null : focused % columns
  const focusedY = focused === null ? null : Math.floor(focused / columns)
  const focusedSector =
    focusedX === null || focusedY === null
      ? null
      : `${String.fromCharCode(65 + Math.floor(focusedX / 4))}${Math.floor(focusedY / 2) + 1}`
    ...
    <div className={`map-shell ${compact ? 'compact-map' : ''}`}>
      ...
      <div className="map-axis map-axis-top" aria-hidden="true">
        {Array.from({ length: 8 }, (_, index) => (
          <span key={index}>X{String(index * 2).padStart(2, '0')}</span>
        ))}
      </div>
      <div className="map-axis map-axis-bottom" aria-hidden="true">
        {Array.from({ length: 8 }, (_, index) => (
          <span key={index}>X{String(index * 2).padStart(2, '0')}</span>
        ))}
      </div>
      <div className="map-axis map-axis-left" aria-hidden="true">
        {Array.from({ length: 5 }, (_, index) => (
          <span key={index}>Y{String(index * 2).padStart(2, '0')}</span>
        ))}
      </div>
      <div className="map-axis map-axis-right" aria-hidden="true">
        {Array.from({ length: 5 }, (_, index) => (
          <span key={index}>Y{String(index * 2).padStart(2, '0')}</span>
        ))}
      </div>
      <svg className="tactical-overlay" viewBox="0 0 160 100" preserveAspectRatio="none" aria-label="Tactical reconnaissance overlay">
        ...
        {[{ x: 16, y: 81 }, { x: 43, y: 63 }, { x: 71, y: 72 }, { x: 91, y: 59 }, { x: 124, y: 30 }, { x: 146, y: 17 }].map(
          (point, index) => (
            <g className={`waypoint waypoint-${index}`} key={`${point.x}-${point.y}`}>
              <circle cx={point.x} cy={point.y} r="2.1" />
              <text x={point.x + 3} y={point.y - 3}>
                WPT-{String(index + 1).padStart(2, '0')}
              </text>
            </g>
          ),
        )}
        {['A1', 'B2', 'C3', 'D4'].map((label, index) => (
          <text className="sector-label" x={22 + index * 38} y={16 + (index % 2) * 70} key={label}>
            {label}
          </text>
        ))}
        <g className={`scan-layer ${scanning ? 'is-scanning' : ''}`}>
          <circle className="scan-radius" cx="91" cy="59" r="23" />
          <path className="scan-sweep" d="M91 59 L113 59 A22 22 0 0 0 102 40 Z" />
        </g>
        <g className="crosshair" transform="translate(91 59)">
          <circle r="5.5" />
          <path d="M-10 0H-6 M6 0H10 M0-10V-6 M0 6V10" />
        </g>
        {(mode === 'diff' || state === 'VERIFYING') && (
          <g className="anomaly-layer">
            <rect x="111" y="28" width="13" height="31" />
            <rect x="70" y="48" width="20" height="9" />
            <text x="125" y="27">ZONE B3</text>
            <text x="71" y="46">ZONE C1</text>
          </g>
        )}
        <g className="overlay-rover" transform="translate(91 59) rotate(18)">
          <path className="rover-heading" d="M0-19 L-4-10 L4-10 Z" />
      ...
      {mode === 'occupancy' && (
        <>
          <div className={`map-status ${scanning ? 'active' : ''}`}>
            <span className="status-marker" />
            {state === 'LEARNING'
              ? 'SCANNING / MAPPING'
              : state === 'VERIFYING'
                ? 'VERIFY ROUTE ACTIVE'
                : 'TACTICAL LINK / STANDBY'}
          </div>
          <div className="map-rover" aria-label="Rover pose" />
          <div className="heading-readout">HDG 018° · TRK 1.2 M/S</div>
          {focusedSector && focusedX !== null && focusedY !== null && (
            <div className="map-readout">
              <strong>{focusedSector}</strong>
              <span>GRID X{String(focusedX).padStart(2, '0')} / Y{String(focusedY).padStart(2, '0')}</span>
              <span>OCCUPANCY {Math.round(cells[focused!] * 100)}%</span>
            </div>
          )}
          <div className="map-legend">
            ...
            <span>
              <i className="legend-swatch legend-ghost" />
              baseline
            </span>
            <span>
              <i className="legend-swatch legend-rover" />
              rover
            </span>
            <span>
              <i className="legend-swatch legend-critical" />
              critical zone
            </span>
            <span>
        ...
        <Panel title="Occupancy / point cloud" code="LIVE MAP" className="map-panel">
          <div className="panel-body" style={{ padding: 0 }}>
            ...
            <MapGrid
              ghost={ghostOn}
              state={state}
              interpolation={state === 'LEARNING' ? Math.max(0.12, progress / 100) : state === 'VERIFYING' ? 1 : 0.72}
            />
            <div
              style={{
                padding: '10px 16px',
...
function Analysis({ state }: { state: RunState }) {
    ...
    <div className="analysis-layout">
      <div className="analysis-main">
        <Panel title="Time-machine map" code="INTERPOLATED OCCUPANCY">
          <div className="panel-body" style={{ padding: 0 }}>
            <MapGrid interpolation={time / 100} state={state} />
    ...
    <div className="app-shell">
      ...
      <div className="body-layout">
        ...
        <main className="main">
          ...
          {view === 'analysis' && <Analysis state={state} />}
